...
import openemr_smart as smart_module
import main as app_module


@pytest.fixture
def smart_client(monkeypatch):
    ...
    global config_module, smart_module, app_module
    config_module = reload(config_module)
    smart_module = reload(smart_module)
...
def test_authorize_url(smart_client):
    r = smart_client.get('/auth/fhir/authorize', headers={'Authorization': f'Bearer {app_module.settings.guest_secret}'})
...
def test_callback_and_internal_jwt(smart_client, monkeypatch):
    fake_response = {
    ...
    with patch('main.exchange_code', side_effect=lambda code, state: {**fake_response, 'session_id': 'sess123'}), \
        ...
        r = smart_client.get('/auth/fhir/callback', params={'code': 'abc', 'state': 'xyz'}, headers={'Authorization': f'Bearer {app_module.settings.guest_secret}'})
        assert r.status_code == 200
        internal = r.json()['access_token']
        assert internal
        me = smart_client.get('/auth/me', headers={'Authorization': f'Bearer {internal}'})
...
def test_scope_enforcement_missing(monkeypatch):
    ...
    with patch('main.get_session_token', return_value={'session_id': 'missing', 'access_token': 'tok', 'scope': ''}):
        ...
        try:
            r = local_client.post('/transcribe/local/', headers={'Authorization': f'Bearer {token}'}, files={'file': ('a.wav', b'data', 'audio/wav')})
        finally:
            local_client.close()
