...
class _MyAppState extends State<MyApp> {
  ...
  final String _issuer = const String.fromEnvironment('KEYCLOAK_ISSUER', defaultValue: 'https://your-keycloak-domain/realms/your-realm');
  ...
  Future<void> _loginWithKeycloak() async {
    try {
      ...
      setState(() { _result = 'Login failed: $e'; });
    }
  }
  ...
  Future<void> _loginAsGuest() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() { _result = 'Guest login failed: ${response.body}'; });
    }
  }
  ...
  Future<http.Response> _retryingPost(Uri uri,{Map<String,String>? headers,Object? body,int retries=2}) async {
    int attempt=0; Duration backoff=const Duration(milliseconds:400);
    while (true) {
      try {
        final resp = await http.post(uri,headers: headers,body: body).timeout(const Duration(seconds:30));
        if (resp.statusCode==401 && attempt==0) { // try refresh once
          await _maybeRefreshToken();
          attempt++; continue;
        }
        if (resp.statusCode>=500 && attempt<retries) {
          await Future.delayed(backoff); backoff*=2; attempt++; continue;
        }
        return resp;
      } catch (_) {
        if (attempt>=retries) rethrow; attempt++; await Future.delayed(backoff); backoff*=2;
      }
    }
  }
  ...
  Future<void> _startAmbientMode() async {
    ...
    if (status.isGranted) {
      ...
      if (available) {
        setState(() { _isListening = true; _ambientText = ''; });
        _speech!.listen(
          onResult: (val) async {
            setState(() { _ambientText = val.recognizedWords; });
  ...
  Future<void> _sendAmbientTranscript(String text) async {
    ...
    if (response.statusCode == 200) {
      setState(() { _result = 'Ambient transcript sent: $text'; });
    } else {
      setState(() { _result = 'Error sending ambient transcript: ${response.body}'; });
  ...
  String _sanitizeForShare(String input) {
    ...
    return input.replaceAllMapped(digitRe, (_) => '***').replaceAllMapped(emailRe, (_) => '***');
  }
  ...
  Future<void> _checkBandwidthAndSetMode() async {
    ...
    _bandwidth = 1000;
    final response = await http.get(Uri.parse('$BASE_URL/network_advice/?bandwidth_kbps=$_bandwidth'));
    if (response.statusCode == 200) {
  ...
  Future<void> _uploadAndTranscribe(File file) async {
    setState(() { _isUploading = true; _result = 'Uploading...'; });
  ...
  endpoint = '/transcribe/?use_cloud=true';
    ...
    if (_selectedMode == 'wifi') {
      ...
      final response = await streamedResponse.stream.transform(utf8.decoder).join();
      setState(() { _result = response; _isUploading = false; });
    } else {
      final response = await http.Response.fromStream(streamedResponse);
      if (response.statusCode == 200) {
        setState(() { _result = json.decode(response.body)['text'] ?? response.body; });
      } else {
          ...
          child: !_isLoggedIn
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        Checkbox(
                          value: _phiConsent,
                          onChanged: (v){ setState(()=>_phiConsent = v??false); },
                        ),
            ...
            ElevatedButton(
              ...
              FilePickerResult? result = await FilePicker.platform.pickFiles(type: FileType.audio);
                          if (result != null && result.files.single.path != null) {
                            setState(() {
                              _savedAudioPath = result.files.single.path;
                              _result = 'Audio recorded. You can transcribe it later.';
                            });
                          } else {
                            setState(() { _result = 'No file selected.'; });
                          }
                    ...
                    const SizedBox(height: 20),
                    _isUploading ? const CircularProgressIndicator() : Text(_result),
                    if (_result.isNotEmpty && !_isUploading && _transcriptionType == 'realtime' && _phiConsent) ...[
                      const SizedBox(height: 20),
                      ElevatedButton(
                        onPressed: () async {
                          await Printing.layoutPdf(
                            onLayout: (format) async => await Printing.convertHtml(
                              format: format,
                              html: '<h1>Transcription Result</h1><p>${_sanitizeForShare(_result).replaceAll("\n", "<br>")}</p>',
                            ),
                          );
                        },
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          final subject = Uri.encodeComponent('Transcription Result');
                          final body = Uri.encodeComponent(_sanitizeForShare(_result));
